...
async def get_list(session: httpx.AsyncClient(), init: bool = False):
    ...
    for a in data.xpath('/html/body/div/div[2]/div/div/a'):
        ...
        if not Products.get(name):
            ...
            if not init:
                await bot.cqbot.send_private_msg(2086378741, f'新增 {name} 项')
    ...
    try:
        ...
        if val['option']:
            if not val['option'] == content:
                ...
                if new:
                    msg += '上新：' + ', '.join(new) + '\n'
                await bot.cqbot.send_private_msg(2086378741, msg.strip())
                bot.info(msg.strip(), 'acrnm')
        val['option'] = content
    except Exception as e:
        bot.error(f'{name} error: {e}', 'acrnm')
...
# 响应来自 cqbot 的查询命令
@bot.cqbot.setResponse(command='/acrnm')
